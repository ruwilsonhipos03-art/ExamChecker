...
def detect_bubble_grid(img, filename):
    try:
        ...
        # --- QR CODE DETECTION (ROBUST) ---
        qr_data = None
        h, w = img.shape[:2]

        # QR is upper-left AFTER rotation
        qr_crop = img[
            int(0.02 * h):int(0.35 * h),
            int(0.02 * w):int(0.35 * w)
        ]

        # 1️⃣ Try pyzbar on cropped region
        codes = decode(qr_crop)
        if codes:
            qr_data = codes[0].data.decode("utf-8").strip()
        else:
            # 2️⃣ Fallback to OpenCV QR detector
        ...
        for col in range(cols):
            ...
            def detect_bubbles_in_rect(sub_gray, approx_d):
            ...
            candidates = detect_bubbles_in_rect(col_gray, approx_bubble_w)
            ...
            # fallback if few bubbles detected
        ...
        for (sx1, sy1, sx2, sy2, r_idx, opt_idx, col) in green_boxes:
            ...
            mean_intensity = np.mean(roi) if roi.size > 0 else 255
        ...
        for q_num, opts in questions_dict.items():
            darkest = min(opts, key=lambda x: x["mean_intensity"])
            ans_letter = chr(65 + darkest["opt_idx"])
            final_answers[str(q_num)] = ans_letter
        ...
        debug_folder = os.path.join(project_root, "public", "storage", "debug")
        os.makedirs(debug_folder, exist_ok=True)

        debug_filename = "debug_" + filename
        debug_path = os.path.join(debug_folder, debug_filename)
        cv2.imwrite(debug_path, debug_img)
        relative_path = os.path.join("debug", debug_filename).replace("\\", "/")

        return {"file": filename, "sheet_id": qr_data, "answers": final_answers, "debug": relative_path}
