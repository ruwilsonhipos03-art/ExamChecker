                    ...
                    <form @submit.prevent="saveExam">
                        <div class="modal-body p-4">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea v-model="form.description" rows="3" class="form-control border-2"
                        ...
                        <div v-if="selectedExam" class="row g-4">
                            ...
                            <div v-if="workspaceSection === 'sheets'" class="col-12">
                                <div class="card border-0 shadow-sm mb-4">
                                    ...
                                    <div class="card-body px-4 pb-4">
                                        <div class="mb-3">
                                            <label class="form-label small fw-bold text-secondary">NUMBER OF
                                                SHEETS</label>
                                            <input v-model.number="sheetForm.count" type="number" min="1" max="200"
                                                class="form-control border-2">
                                        </div>
                                        <button @click="generateSheets" class="btn btn-emerald w-100 fw-bold"
                                            :disabled="isGeneratingSheets || !isAnswerKeyReady(selectedExam)">
...
const form = reactive({ Exam_Title: '', Exam_Type: 'Term', description: '', program_id: '' });
const answerKeyForm = reactive({ answers: {} });
const sheetForm = reactive({ count: 1 });
...
const examDescription = (exam) => {
    return String(exam?.description || '').trim() || 'No description';
};
...
const generateSheets = async () => {
    ...
    try {
        const res = await axios.post(
            ...
            { exam_id: Number(selectedExam.value.id), count: Number(sheetForm.count || 1) },
            { responseType: 'blob' }
        );
