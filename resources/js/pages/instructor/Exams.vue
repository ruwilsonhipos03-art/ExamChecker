            ...
            <div class="card-body p-4">
                <div class="row align-items-center">
                    <div class="col">
                        ...
                        <p class="text-muted small mb-0">Create exams, set answer keys, and generate term sheets in one place</p>
                    </div>
                    <div class="col-auto">
            ...
            <div class="card-body p-0">
                <div class="px-4 py-3 border-bottom bg-light">
                    <div class="input-group input-group-sm w-50 w-md-25">
                        ...
                        <input v-model="searchQuery" type="text" class="form-control border-start-0 ps-0" placeholder="Search by title or type...">
                    </div>
                </div>
                    ...
                    <table class="table table-hover mb-0 align-middle">
                        <thead class="bg-light">
                            <tr>
                                ...
                                <th class="py-3 text-secondary small fw-bold">DESCRIPTION</th>
                                <th class="py-3 text-secondary small fw-bold">SUBJECT</th>
                        ...
                        <tbody>
                            ...
                            <template v-else>
                                <tr v-for="(exam, index) in filteredExams" :key="exam.id" class="exam-row" @click="openWorkspace(exam)">
                                    <td class="ps-4 text-muted">#{{ index + 1 }}</td>
                                    <td>
                                        <button class="btn btn-link p-0 text-decoration-none text-start exam-title-button">
                                    ...
                                    <td class="text-muted small">{{ examDescription(exam) }}</td>
                                    <td class="text-muted small">{{ examSubjectName(exam) }}</td>
                                    <td>
                                        <span class="badge rounded-pill bg-light text-emerald border border-emerald px-3">{{ exam.Exam_Type }}</span>
                                    </td>
                                    <td>
                                        <span class="badge rounded-pill px-3 py-2 small fw-semibold" :class="statusBadgeClass(exam)">
                                            {{ examStatusLabel(exam) }}
                                        </span>
                                    </td>
                                    <td class="pe-4 text-end" @click.stop>
                                        <button @click="openExamModal(exam)" class="btn btn-icon btn-light-success me-2" title="Edit">
                                            <i class="bi bi-pencil-square"></i>
                                        </button>
                                        <button @click="deleteExam(exam.id)" class="btn btn-icon btn-light-danger" :disabled="deletingId === exam.id">
                                            <span v-if="deletingId === exam.id" class="spinner-border spinner-border-sm"></span>
                    ...
                    <form @submit.prevent="saveExam">
                        <div class="modal-body p-4">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea v-model="form.description" rows="3" class="form-control border-2" placeholder="Enter exam description"></textarea>
                            </div>
                            <div class="mb-3">
                ...
                <div class="modal-content border-0 shadow-lg">
                    <div class="modal-header bg-dark text-white border-0">
                        <div>
                            ...
                            <p class="mb-0 small text-white-50">{{ selectedExamSubjectName }}</p>
                        </div>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                        ...
                        <div v-if="selectedExam" class="row g-4">
                                    ...
                                    <div class="card-body">
                                        <div class="d-flex flex-column flex-lg-row justify-content-between gap-3">
                                            <div>
                                                ...
                                                <span class="badge rounded-pill px-3 py-2 small fw-semibold" :class="statusBadgeClass(selectedExam)">
                                                    {{ examStatusLabel(selectedExam) }}
                                                </span>
                                            </div>
                                            <div class="text-muted small">Term sheet generation is enabled only after a complete answer key is saved.</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-12">
                                <div class="selection-switcher">
                                    <button type="button" class="selection-pill" :class="{ active: workspaceSection === 'answers' }" @click="workspaceSection = 'answers'">Answer Key</button>
                                    <button v-if="isAnswerKeyReady(selectedExam) && selectedExamSubjectId" type="button" class="selection-pill" :class="{ active: workspaceSection === 'sheets' }" @click="workspaceSection = 'sheets'">Generate Sheets</button>
                                </div>
                            </div>
                            ...
                            <div v-if="workspaceSection === 'answers'" class="col-12">
                                <div class="card border-0 shadow-sm h-100">
                                    <div class="card-header bg-white border-0 pt-4 px-4 d-flex justify-content-between align-items-center">
                                        ...
                                        <button v-if="selectedAnswerKey" @click="deleteAnswerKey" class="btn btn-light-danger btn-sm" :disabled="isDeletingAnswerKey">
                                            <span v-if="isDeletingAnswerKey" class="spinner-border spinner-border-sm me-2"></span>
                                            Delete Key
                                        </button>
                                    </div>
                                            ...
                                            <div v-for="n in 100" :key="n" class="answer-grid-item">
                                                <div class="answer-box compact-box" :class="getAnswerBoxClass(n)">
                                                    ...
                                                    <div class="p-1 compact-select-wrap">
                                                        <select v-model="answerKeyForm.answers[n]" class="form-select form-select-sm border-0 shadow-sm text-center fw-bold compact-select" :class="getAnswerSelectClass(n)" @focus="focusedItem = n" @change="markAnswerTouched(n)">
                                                            <option value="">-</option>
                                                            <option v-for="opt in ['A', 'B', 'C', 'D', 'E']" :key="opt" :value="opt">{{ opt }}</option>
                                                        </select>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <div v-if="workspaceSection === 'answers'" class="col-12">
                                <div class="d-flex justify-content-end">
                                    <button @click="saveExamConfiguration" class="btn btn-emerald px-4 fw-bold" :disabled="isSavingConfiguration || !selectedExam || !canSaveExamConfiguration">
                                        <span v-if="isSavingConfiguration" class="spinner-border spinner-border-sm me-2"></span>
                            ...
                            <div v-if="workspaceSection === 'sheets'" class="col-12">
                                <div class="card border-0 shadow-sm mb-4">
                                    <div class="card-header bg-white border-0 pt-4 px-4">
                                        ...
                                        <p class="text-muted small mb-0">Generate printable term sheets for this exam and subject.</p>
                                    </div>
                                    <div class="card-body px-4 pb-4">
                                        <div class="small text-muted mb-3">Assigned students: {{ selectedSubjectCount }}</div>
                                        <button @click="generateSheets" class="btn btn-emerald w-100 fw-bold" :disabled="isGeneratingSheets || !isAnswerKeyReady(selectedExam) || !selectedExamSubjectId">
                                            <span v-if="isGeneratingSheets" class="spinner-border spinner-border-sm me-2"></span>
                                ...
                                <div class="card border-0 shadow-sm">
                                    <div class="card-header bg-white border-0 pt-4 px-4">
                                        ...
                                        <p class="text-muted small mb-0">Manage sheets already created for this exam.</p>
                                    </div>
                                    <div class="card-body px-4 pb-4">
                                        <div v-if="selectedExamSheets.length === 0" class="text-muted small">No answer sheets generated yet.</div>
                                        <div v-else class="sheet-list">
                                            <div v-for="sheet in selectedExamSheets" :key="sheet.id" class="sheet-item">
                                                <div>
                                                    <div class="fw-semibold small text-dark">{{ sheet.qr_payload }}</div>
                                                    <div class="text-muted x-small text-uppercase">{{ sheet.status }}</div>
                                                </div>
                                                <div class="d-flex gap-2">
                                                    <button @click="downloadSheet(sheet.id)" class="btn btn-light-success btn-sm" :disabled="printingId === sheet.id || deletingSheetId === sheet.id">
                                                        <span v-if="printingId === sheet.id" class="spinner-border spinner-border-sm"></span>
                                                        <i v-else class="bi bi-download"></i>
                                                    </button>
                                                    <button @click="deleteSheet(sheet.id)" class="btn btn-light-danger btn-sm" :disabled="deletingSheetId === sheet.id || printingId === sheet.id">
                                                        <span v-if="deletingSheetId === sheet.id" class="spinner-border spinner-border-sm"></span>
                                                        <i v-else class="bi bi-trash3"></i>
                                                    </button>
                                                </div>
                                            </div>
...
const examSubjectRow = (exam) => {
    const rows = exam?.exam_subjects || exam?.examSubjects || [];
    return Array.isArray(rows) && rows.length > 0 ? rows[0] : null;
};

const filteredExams = computed(() => {
    const allowed = (exams.value || []).filter((exam) =>
        EXAM_TYPE_ALIASES.includes(String(exam?.Exam_Type || '').trim().toLowerCase())
    );
    return allowed.filter((exam) =>
        [
            String(exam?.Exam_Title || ''),
            String(exam?.description || ''),
            examSubjectName(exam),
            String(exam?.Exam_Type || ''),
        ].some((value) => value.toLowerCase().includes(searchQuery.value.toLowerCase()))
    );
});

const examDescription = (exam) => String(exam?.description || '').trim() || 'No description';
const examSubjectName = (exam) =>
    String(examSubjectRow(exam)?.subject?.Subject_Name || examSubjectRow(exam)?.subject?.subject_name || '').trim() || 'No subject assigned';

const selectedExamSubjectId = computed(() => {
    const subjectId = examSubjectRow(selectedExam.value)?.subject_id;
    return subjectId ? Number(subjectId) : null;
});

const selectedExamSubjectName = computed(() => examSubjectName(selectedExam.value));

const selectedSubjectCount = computed(() => {
    const subject = subjects.value.find((row) => Number(row?.id) === Number(selectedExamSubjectId.value));
    return Number(subject?.students_count || 0);
});
...
const answerKeyValidationError = computed(() =>
    hasCompleteAnswers() ? '' : 'Please complete all 100 answer items before saving.'
);
...
const fetchExams = async () => {
    try {
        const response = await axios.get('/api/exams', { params: { scope: 'term' } });
...
const fetchSubjects = async () => {
    try {
        const response = await axios.get('/api/instructor/subjects');
        subjects.value = extractArray(response.data).map((row) => ({
            id: row.id,
            subject_name: row.Subject_Name || row.subject_name || 'Subject',
...
const refreshWorkspaceData = async () => {
    ...
    try {
        await Promise.allSettled([fetchExams(), fetchAnswerKeys(), fetchSheets(), fetchSubjects()]);
...
const openExamModal = (exam = null) => {
    ...
    const subjectRow = examSubjectRow(exam);
    if (subjectRow) {
        form.subject_id = subjectRow.subject_id ? String(subjectRow.subject_id) : '';
        form.Starting_Number = Number(subjectRow.Starting_Number || 1);
        form.Ending_Number = Number(subjectRow.Ending_Number || 100);
    }
...
const generateSheets = async () => {
    if (!selectedExam.value || !selectedExamSubjectId.value) return;
    isGeneratingSheets.value = true;
    try {
        const res = await axios.post(
            '/api/answer-sheets/generate-term',
            { exam_id: Number(selectedExam.value.id), subject_id: Number(selectedExamSubjectId.value) },
            { responseType: 'blob' }
        );
        ...
        await refreshWorkspaceData();
        window.Toast.fire({ icon: 'success', title: 'Term exam sheets generated and downloaded.' });
    } catch (e) {
